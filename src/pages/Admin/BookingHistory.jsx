...
const groupByMonthYear = (bookings) => {
  ...
  bookings.forEach((booking) => {
    const date = new Date(booking.registrationDate);
    ...
    <div className="min-h-screen bg-gray-900 text-white p-4">
      ...
      >
        ...
        {loading ? (
          ...
          <div>
            {Object.entries(groupByMonthYear(users)).map(([monthYear, bookings]) => (
              <div key={monthYear} className="mb-10">
                ...
                <div className="block md:hidden space-y-4">
                  {bookings.map((user, index) => (
                    ...
                    >
                      <p>
                        <span className="text-blue-400 font-semibold">User ID:</span>{" "}
                        {user.userID}
                      </p>
                      <p>
                      ...
                      <p>
                        <span className="text-blue-400 font-semibold">Registered:</span>{" "}
                        {new Date(user.registrationDate).toLocaleDateString()}
                      </p>
                      <p>
                        <span className="text-blue-400 font-semibold">Expiry:</span>{" "}
                        {new Date(user.expiryDate).toLocaleDateString()}
                      </p>
                      <p>
                      ...
                      <p>
                        <span className="text-blue-400 font-semibold">Price:</span> ₹{user.price}
                      </p>
                    </motion.div>
                  ))}
                </div>
